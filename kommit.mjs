...
async function askUserForTemplate() {
  ...
  const chosenTemplateId = await $`cat ${files.static}`.pipe`${[
    ...
    "--reverse",
    ...["--accept-nth", "1"],
    ...["--bind", `ctrl-d:reload(cat ${files.static})+change-prompt(static>)`],
    ...["--bind", `ctrl-l:reload(cat ${files.log})+change-prompt(log>)`],
    ...["--bind", `ctrl-r:reload(cat ${files.reflog})+change-prompt(reflog>)`],
